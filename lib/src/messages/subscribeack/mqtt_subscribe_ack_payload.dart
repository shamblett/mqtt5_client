...
class MqttSubscribeAckPayload extends MqttIPayload {
  ...
  void readFrom(MqttByteBuffer payloadStream) {
    ...
    for (var i = 0; i < payloadLength; i++) {
      _reasonCodes.add(
        MqttSubscribeReasonCodeSupport.mqttSubscribeReasonCode.fromInt(payloadStream.readByte()),
      );
      _length += 1;
    }
  }
  ...
  String toString() {
    ...
    for (final value in _reasonCodes) {
      if (value != null) {
        sb.writeln('Reason Code = ${MqttSubscribeReasonCodeSupport.mqttSubscribeReasonCode.asString(value)}');
      }
    }
    return sb.toString();
  }
