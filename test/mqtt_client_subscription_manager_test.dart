...
void main() {
  ...
  group('Subscription Class', () {
    ...
    test('Equality', () {
      ...
      final subscription3 = MqttSubscription.withMaximumQos(
        MqttSubscriptionTopic('a/nother/topic'),
        MqttQos.atLeastOnce,
      );
  ...
  group('Subscription Manager - Common', () {
    group('Construction', () {
      test('Default Construction', () {
        ...
        expect(
          subs.getSubscriptionTopicStatus('a/topic'),
  ...
  group('Subscription Manager - Subscribe', () {
    group('Subscribe Functionality - Pending', () {
      ...
      test('Subscription List request creates pending subscriptions', () {
        ...
        final subscription1 = MqttSubscription(
          MqttSubscriptionTopic('topic1'),
        ...
        final subscription2 = MqttSubscription(
          MqttSubscriptionTopic('topic2'),
          MqttSubscriptionOption()..maximumQos = MqttQos.atMostOnce,
        );
        final subscription3 = MqttSubscription(
          MqttSubscriptionTopic('topic3'),
        ...
        expect(
          msg.payload!.subscriptions[0].option.maximumQos,
          MqttQos.atLeastOnce,
        );
        expect(msg.variableHeader!.userProperty, isNotNull);
        expect(msg.variableHeader!.userProperty.length, 1);
        ...
        expect(
          msg.payload!.subscriptions[1].option.maximumQos,
          MqttQos.atMostOnce,
        );
        expect(msg.payload!.subscriptions[2].topic.rawTopic, 'topic3');
        expect(
          msg.payload!.subscriptions[2].option.maximumQos,
          MqttQos.exactlyOnce,
        );
  ...
  group('Subscription Manager - Unsubscribe', () {
    ...
    group('Unsubscribe Functionality - Acknowledge', () {
      test(
        'Acknowledged unsubscription request removes active subscription',
        () {
          var cbCalled = false;
  ...
  group('Resubscribe on auto reconnect', () {
    test('Active', () async {
      ...
      final subs = MqttSubscriptionManager(testCHS, clientEventBus);
      subs.messageIdentifierDispenser.reset();
      subs.resubscribeOnAutoReconnect = true;
      final subscription = MqttSubscription(
        MqttSubscriptionTopic(topic),
        MqttSubscriptionOption()..maximumQos = qos,
      );
      ...
      const topicpending = 'testtopicpending';
      final subscriptionpending = MqttSubscription(
        MqttSubscriptionTopic(topicpending),
        MqttSubscriptionOption()..maximumQos = qos,
      );
      subs.subscribeSubscription(subscriptionpending);
      expect(
    ...
    test('InActive', () async {
      ...
      final subs = MqttSubscriptionManager(testCHS, clientEventBus);
      subs.messageIdentifierDispenser.reset();
      subs.resubscribeOnAutoReconnect = false;
      final subscription = MqttSubscription(
        MqttSubscriptionTopic(topic),
        MqttSubscriptionOption()..maximumQos = qos,
      );
